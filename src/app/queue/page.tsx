...
export default function QueuePage() {
	...
	function handleEditStart(item: QueueItem) {
		...
		setEditingText(item.draft_reply);
	}
			...
			<div className="mb-6">
				...
				<div>
					...
					<p className="text-zinc-400 mt-1 text-sm">
						{pendingCount} oczekujących · {items.length} łącznie
					</p>
				</div>
			</div>
							...
							{editingId === item.id ? (
								<div className="flex flex-col gap-2">
									<textarea
										...
										rows={4}
										className="w-full bg-zinc-900 border border-zinc-600 rounded-lg px-3 py-2 text-sm text-zinc-200 outline-none focus:border-zinc-400 resize-none"
									/>
